# conversation-index Specification

## Purpose

The conversation-index capability defines the service-worker indexing pipeline that turns a platform adapter's read messages into normalized, searchable `ConversationIndex` records. It owns content normalization, idempotent re-indexing via `contentHash`, preservation of local organization state across re-indexing, and chunked best-effort bulk indexing. `ConversationIndex`, its `indexedText`, and the resulting postings are local-only and never enter the sync envelope.
