# adapter-claude Specification

## Purpose

The adapter-claude capability defines the bundled Claude platform integration built entirely on the generic config-driven adapter: a schema-valid `AdapterConfig` for Claude that drives its React-controlled composer with no platform-specific code, validation that the config passes the shared adapter contract suite against recorded Claude DOM fixtures, and confirmation that `selfCheck()` degrades cleanly (reporting missing anchors without throwing) when run against a broken Claude fixture.
