...
The observability capability defines the extension's diagnostics-only telemetry: an opt-in,
off-by-default consent gate; a single service-worker egress path; a closed event taxonomy with
allowlisted properties enforced by a fake-transport test; no per-user identity (a fixed anonymous
`distinct_id`); scrubbed exception reporting in PostHog's Error Tracking shape; durable, alarm-driven
batched egress that survives worker death; instant local-authoritative opt-out; and a single SDK-less
PostHog EU endpoint with no remote code. It collects crashes and adapter health only — never usage or
product analytics, and never user content.
