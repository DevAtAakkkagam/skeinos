# ui-interaction-primitives Specification

## Purpose

The ui-interaction-primitives capability defines a reusable, framework-agnostic, shadow-DOM-aware interaction layer for the overlay's floating and overlay widgets. It wraps `@floating-ui/dom` into a positioning helper (offset/flip/shift, with the overlay's shadow root as the positioning context) and Zag.js interaction state machines into a Preact bridge, and ships two accessible widgets built on them — a menu and a modal dialog. Every floating, portaled, or overlay element this layer produces renders inside the overlay's shadow root so it inherits the `--sk-*` tokens and stays isolated from host-page CSS. This layer is the designated home for later floating widgets (search command palette, tag picker, prompt library, tooltips) so they build on a common accessible foundation rather than re-solving positioning and ARIA by hand.
