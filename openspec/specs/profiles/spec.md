...
The profiles capability defines the on-device instruction-profile library: a syncable profile
record (name, optional description, instruction text, the platforms it applies to, and an optional
response style), single-writer mutations that stamp the sync envelope and broadcast on change, a
Profiles view with an editor that is a pure view over the worker, idempotent domain-based seeding
of starter profiles, and a per-platform mode indicator that reflects the implemented (PREPEND-only)
injection behavior.
