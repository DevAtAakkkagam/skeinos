...
- Define the typed `Repo<T>` interface (`get`/`put`/`delete`/`query`) and a `WorkspaceStore` facade exposing one repo per store (`folders`, `conversations`, `prompts`, `promptFolders`, `profiles`, `tags`, `comparisons`) plus `searchPostings` and `syncMeta` (LLD §5–§6).
...
- **New module** `extension/src/core/store/` and `shared/` type defs for the records (LLD §6) + the `SyncMeta` envelope.
