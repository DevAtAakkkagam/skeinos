...
a store release (CLAUDE.md [ADAPT]; LLD §4). The `messaging` capability already
defines `PlatformId` and the `platform.degraded` broadcast, so this change
*consumes* the bus rather than extending it.

This is the M1 critical-path change (LLD T1.1–T1.4). It is the first code that
...
**Goals:**
- A `PlatformAdapter` interface (LLD §4.1) that is the *only* platform-facing
  contract the rest of the system imports.
- An `AdapterConfig` schema (LLD §4.2) + runtime validator.
...
- Full host docking/reflow coexistence (LLD §4.4) — lands with the P0 platforms
...
keeps `PlatformAdapter` the sole external contract (LLD §4.1, [ADAPT]).
...
*Rejected for now* — the schema is small and fixed (LLD §4.2), remote config is
...
offline, and identical for every future platform (LLD §12 "adapter contract
...
`submitMode: 'click' | 'enter'` (LLD §4.2) let the generic adapter handle
