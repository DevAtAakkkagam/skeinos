...
- [ ] 1.1 Add `onboardingCompleted: boolean` and `domain?: DomainId` to the `Settings`
...
- [ ] 1.2 Add `onboardingCompleted: false` to `DEFAULT_SETTINGS` (leave `domain` absent so it
...
- [ ] 2.1 Create `ui/onboarding/gate.ts`: an `isOnboardingComplete(settings)` selector and an
      async `completeOnboarding()` writer that sets `onboardingCompleted: true` via `setSettings`.
- [ ] 2.2 Create `ui/onboarding/OnboardingSurface.tsx`: a minimal `.sk-shell` placeholder with a
...
- [ ] 3.1 In `SidePanelApp.tsx`, resolve onboarding state from settings (`getSettings` +
      `subscribeSettings`), tracking an unresolved (`undefined`) phase distinct from
      complete/not-complete.
- [ ] 3.2 Add the router branch ABOVE the existing platform branch: render `<OnboardingSurface/>`
...
- [ ] 3.3 Ensure completing onboarding re-scopes the panel live via the `subscribeSettings`
      subscription (no reload), and dispose the subscription on unmount.

## 4. Tests (authored by a sub agent)

- [ ] 4.1 Settings defaults: fresh read yields `onboardingCompleted === false` and `domain`
...
- [ ] 4.2 Gate helpers: `isOnboardingComplete` reflects the flag; `completeOnboarding` writes
      `onboardingCompleted: true` through `setSettings`.
- [ ] 4.3 Panel branch precedence: not-complete renders the onboarding surface with AND without a
...
- [ ] 4.4 Live re-scope: invoking the surface's action marks onboarding complete and the panel
      leaves the onboarding surface without a reload; completion persists across a reload.

## 5. Verification

- [ ] 5.1 Run `npm run typecheck` and `npm test`; then `npm run test:browser` for the panel
